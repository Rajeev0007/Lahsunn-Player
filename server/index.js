...
async function handleApi(req, res) {
  ...
  if (p === "/api/search") {
    ...
    if (!query) {
      return json(res, 200, { tracks: [], albums: [], artists: [], playlists: [] });
    }

    console.log(`[SEARCH] Query: ${query}`);
    console.log(`[SEARCH] Lavalink base: ${LL_BASE}`);
    console.log(`[SEARCH] Host configured: ${Boolean(LL_HOST)}`);
    console.log(`[SEARCH] Auth configured: ${Boolean(LL_AUTH)}`);
    ...
    try {
      if (/^https?:\/\//i.test(query)) {
        ...
        if (col.info) {
          ...
          console.log(`[SEARCH] URL result: ${payload.tracks.length} tracks`);
          return json(res, 200, payload);
        }
        console.log(`[SEARCH] URL result: ${col.tracks.length} tracks`);
      ...
      for (const [name, attempt] of [
        ...
        if (attempt.status === "fulfilled") {
          const count = Array.isArray(attempt.value)
            ? attempt.value.length
            : attempt.value?.tracks?.length || 0;
          console.log(`[SEARCH] ${name}: success (${count})`);
        } else {
          console.error(`[SEARCH] ${name}: failed`, attempt.reason?.message || attempt.reason);
        }
      }
      ...
      console.log(
        `[SEARCH] Final: ${result.tracks.length} tracks, ${result.albums.length} albums, ${result.artists.length} artists, ${result.playlists.length} playlists`
      );
      ...
      if (!result.tracks.length && !result.albums.length && !result.artists.length && !result.playlists.length) {
        ...
        if (errors.length === attempts.length) {
          return json(res, 502, {
            error: "All Lavalink search attempts failed",
            details: errors,
            tracks: [],
            albums: [],
            artists: [],
      ...
      console.error("[SEARCH] Unexpected failure:", error);
      return json(res, 500, {
        error: "Search failed",
